...
public interface OrderDAO extends JpaRepository<Orders, Long> {
	@Query(value = "select sum(total_money) from order where (date_ship = ?1 and state = 1 ) ", nativeQuery = true)
	...
	@Query(value = "select sum(total_money) from order where (id_staff_order = ?1 and state = 1)", nativeQuery = true)
	...
	@Query(value = "select sum(quantity) from detail_order where \n" + 
				   "id_product = ?1 and\n" + 
				   "id_order in (select id from order where datediff(curdate(), date_ship) <30 "
